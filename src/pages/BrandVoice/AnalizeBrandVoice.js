...
function AnalizeBrandVoice({ setIsEdit = () => {}, setIsAnalize = () => {} }) {
			...
			<div className="w-full flex">
				<div className="w-full flex flex-col relative space-y-3">
					<div className="w-full">
						<div className="w-full my-2 text-xs text-slate-400 text-right">
							({t("required")}){t("nameBrandVoice")}
						</div>
						<input
							type="text"
					...
					<div className="w-full mb-2 text-xs text-slate-400 text-right">
						{t("brandVoice")}
					</div>
					<textarea
					...
					<div>
						<div className="flex w-max flex-nowrap rounded-xl border-none text-xs transition-all duration-300 items-center max-w-full relative border-purple-300 bg-purple-100 text-purple-900 opacity-100 mb-1 border py-3 px-4  mt-4">
							<span className="w-full text-sm line-clamp-6 text-right">
								{t("tipBrandVoice")}
					...
					<div className="flex justify-start items-start cursor-pointer py-2">
						<span className="ml-6">{t("saveAsDefaultVoice")}</span>
					...
					<div className="flex pt-6 justify-end">
						...
						<button
							...
							disabled=""
							tabindex="0"
							className="group focus:outline-none flex select-none items-center text-xs rounded-lg px-3 py-2 font-semibold focus:ring-purple-500 hover:bg-purple-800 pointer-events-none bg-slate-200 border-slate-300 text-slate-700 ml-2 ring-offset-2 focus:ring-2"
						>
							<h3
								className="flex-nowrap whitespace-nowrap ml-2 text-h3 font-semibold tracking-wide text-left"
								aria-label="Save Brand Voice"
